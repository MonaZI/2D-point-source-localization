...
snr_vec = [1000,100,10,1,0.1]; %signal to noise ratio, %for clean data replace with snr = 'clean'
...
for k_ind = 1:length(k_vec)
    ...
    for snr_ind = 1:length(snr_vec)
        ...
        for iter = 1:num_iter
            ...
            % generate points
            xx = point_source_generator_pair(k, 0.1).';
            x = xx*R;
            ...
            for L_ind = 1:length(L_vec)
                ...
                [ ~, F_c ] = feature_from_proj_uniform(disc_projs, X, n_var);
                ind = [start_r:start_r+M_prony-1]';
                r_interval = ind;
                b_pairwise = F_c(ind+1);
                ...
                for nn = 1:length(D)
                    c_approx = c_approx + ...
                        ((1-1j)*exp(2j*pi*r_interval*D(nn)/(delta*M))+ ...
                        (1+1j)*exp( -2j*pi*r_interval*D(nn)/(delta*M) ) )./(2*sqrt(pi*2*pi*D(nn)*r_interval/(delta*M)));
                end
                
                [prony_mat, prony_vec] = gen_prony_mat(((b_pairwise-k)/2).*sqrt(r_interval),length(D)*2);
                c = pinv(prony_mat)*prony_vec;
                c = [1;c];
                r_radial = roots((c));
                
                true_harmonics_radial = exp(2j*pi*(D)/(delta*M));
                ...
                r_radial = r_radial(I(1:length(D)));
                r_rec = (angle(r_radial)*delta*M)/(2*pi);
                
                error_r_MSE(k_ind,L_ind,snr_ind,iter) = norm(sort(r_rec)-sort(D),'fro');
                error_r_rel(k_ind,L_ind,snr_ind,iter) = norm(sort(r_rec)-sort(D),'fro')/norm(rk,'fro');
